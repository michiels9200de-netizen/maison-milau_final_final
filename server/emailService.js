...
const WEBOWNER_EMAIL = (process.env.ADMIN_EMAIL || "maisonmilau@gmail.com").trim();
const SENDER_EMAIL = (process.env.SENDER_EMAIL || process.env.SUPPORT_EMAIL || "maisonmilau@gmail.com").trim();
const rawSenderName = process.env.SENDER_NAME;
const SENDER_NAME = (rawSenderName && rawSenderName !== "Maison Milai" ? rawSenderName : "Maison Milau").trim();
const emailNotificationLogs = [
  {
    id: "eml-init-1",
...
async function sendReviewRequestEmail(order) {
  ...
  const coffeeNames = (order.items || []).map((i) => i.productName).join(", ");
